...
Timer.prototype.speakTime = function() {
  ...
  // "5 minutes in!"
  } else if (lapsed.seconds() === 0 && _([5, 10, 15]).include(lapsed.asMinutes())) {
    this.say(lapsed.asMinutes() + " minutes in!");

  // "3 minutes to go!"
  } else if (remaining.seconds() === 0 && _([5, 3, 2, 1]).include(remaining.asMinutes())) {
    this.say(remaining.humanize() + " to go!");

  // "last X seconds..."
  } else if (_([30, 15]).include(remaining.asSeconds())) {
