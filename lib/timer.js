...
Timer.prototype.speakTime = function() {
  ...
  if (lapsed.asSeconds() === 0) {
    var now = moment().format("H:mm a");
    this.say("it's now " + now + ". let's do " + remaining.humanize());

  } else if (lapsed.seconds() === 0 && [5, 10, 15].indexOf(lapsed.asMinutes()) > -1) {
  ...
  } else if (remaining.asSeconds() < 5 && remaining.asSeconds() >= 0) {
    this.say(parseInt(remaining.asSeconds() + 1, 10));
  }
};
